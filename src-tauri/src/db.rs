use crate::{
  state::{
    now_rfc3339, DEFAULT_PAPER_FILTER_PROMPT, DEFAULT_PAPER_READING_PROMPT,
    DEFAULT_WORK_REPORT_PROMPT,
  },
  types::{
    PaperCardDto, PaperDecision, PaperRecommendationResult, PersistPaper, RuleDto,
    RuntimeSettingDto, RuntimeSettingRow, RuntimeSettingUpsertInput, WorkflowListItem,
  },
};
...
pub fn to_runtime_setting_dto(row: Option<RuntimeSettingRow>) -> RuntimeSettingDto {
  ...
  RuntimeSettingDto {
    ...
    ocr_base_url: row.ocr_base_url.unwrap_or_default(),
    ocr_api_key: row.ocr_api_key.unwrap_or_default(),
    ocr_model: row.ocr_model.unwrap_or_default(),
