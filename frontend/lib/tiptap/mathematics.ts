...
export type InlineDisplayMathematicsOptions = {
  katexOptions?: KatexOptions;
  shouldRender: (state: EditorState, pos: number, node: ProseMirrorNode) => boolean;
};

const DISPLAY_MATH_REGEX = /^\s*\$\$([^$\n]+)\$\$\s*$/;
...
function detectMathMatches(text: string): MathMatch[] {
  ...
  while ((matched = INLINE_MATH_REGEX.exec(text))) {
...
function getAffectedRange(
  ...
  if (previousPluginState.isEditable !== isEditable) {
    ...
    minFrom = Math.min($from.depth === 0 ? 0 : $from.before(), $newFrom.depth === 0 ? 0 : $newFrom.before());
...
export const InlineDisplayMathematics = Extension.create<InlineDisplayMathematicsOptions>({
  name: 'InlineDisplayMathematics',

  addOptions() {
    return {
      katexOptions: undefined,
      shouldRender: (state: EditorState, pos: number) => {
        ...
        state: {
          ...
          apply(tr, previousPluginState, state, newState) {
            ...
            newState.doc.nodesBetween(minFrom, maxTo, (node, pos) => {
              if (!node.isText || !node.text || !shouldRender(newState, pos, node)) {
                return;
              }
                    ...
                    Decoration.widget(
                      ...
                      () => {
                        ...
                        if (match.displayMode) {
                          element.classList.add('Tiptap-mathematics-render--display');
                        }
                        if (isEditable) {
