...
- **后端**：FastAPI + DuckDB（原生 SQL Repository）
- **数据库**：DuckDB（单文件 `persist/deeplab.duckdb`）
...
├── frontend/               # Next.js 前端
├── desktop/                # Electron 桌面宿主与打包脚本（Windows）
├── docs/                   # 模块说明文档
...
```bash
DEEPLAB_DB_PATH=/app/persist/deeplab.duckdb
```
...
- `persist/deeplab.duckdb`：DuckDB 数据文件
...
export DEEPLAB_DB_PATH="$(pwd)/persist/deeplab.duckdb"
export APP_HOST=0.0.0.0
export APP_PORT=8000
export APP_RELOAD=1
export FASTEMBED_CACHE_PATH="$(pwd)/persist/fastembed"
export DEEPLAB_EMBED_CACHE_DIR="$(pwd)/persist/fastembed"
export DEEPLAB_TMP_DIR="$(pwd)/persist/tmp"

# 首次执行可手动运行数据库迁移
uv run python -m deeplab.db.migrate
...
适合内测分发，支持 Windows / macOS / Linux 打包。

#### 1. 环境要求（构建机）

- Node.js `20+`
- uv
- Python `3.12+`（推荐；如由 uv 自动管理 Python 可不单独预装）
- npm
...
桌面版将所有数据写入 Electron `userData/deeplab` 目录：
...
- CI 工作流：`.github/workflows/desktop-prerelease.yml`
- Release 由 GitHub Actions 手动触发（`workflow_dispatch`），并发布为 pre-release。
- 所有平台构建均不进行签名。
- 打包后的桌面应用使用内置运行配置，不依赖自定义环境变量（开发模式除外）。
- 桌面后端运行时环境由 `uv sync --locked --no-dev` 构建。
- 首版不包含自动更新，不包含历史 `persist/` 自动迁移。

## 生产部署建议

- 使用环境变量管理密钥，不要将真实密钥写入仓库。
- 后端建议 `APP_RELOAD=0`，并通过进程管理器或容器重启策略托管。
- 单实例部署下将 `DEEPLAB_DB_PATH` 指向持久化磁盘目录，避免多进程并发写同一文件。
...
- 定期备份 `persist/deeplab.duckdb`。

## PostgreSQL 历史数据迁移

若已有 PostgreSQL 历史数据，可执行一次性导入：
...
uv run python scripts/migrate_postgres_to_duckdb.py --yes
```
